...
export default function Home() {
  const [settings, setSettings] = React.useState({
    ...
    batchSize: 4, // Updated for better performance
  });
  ...
  // Updated analyzeVideo with new backend
  const analyzeVideo = useCallback(async (videoBlob: Blob) => {
    ...
    try {
      // Updated API URL for new backend
      const API_BASE_URL = 'https://faceit-backend-gpu-180532999413.europe-west4.run.app';
      ...
      // Handle async job response (202 status)
      if (response.status === 202 && data.job_id) {
        // For long videos, poll for results
        await pollForResults(API_BASE_URL, data.job_id);
        return;
      }
  ...
  const pollForResults = async (baseUrl: string, jobId: string) => {
    const maxAttempts = 180; // 30 minutes max
    let attempts = 0;
    
    while (attempts < maxAttempts) {
      try {
        const statusResponse = await fetch(`${baseUrl}/analyze/status/${jobId}`);
        if (!statusResponse.ok) throw new Error('Status check failed');
        
        const status = await statusResponse.json();
        
        if (status.status === 'completed') {
          const resultResponse = await fetch(`${baseUrl}/analyze/result/${jobId}`);
        ...
        recordingFlow.setErrorMessage(`Polling failed: ${error instanceof Error ? error.message : String(error)}`);
        recordingFlow.setFlowState("ready_to_start");
        return;
      }
    }
    
    recordingFlow.setErrorMessage("Analysis timeout - video too long");
    recordingFlow.setFlowState("ready_to_start");
  };
