...
{ spawnTime: 1500, displayTime: 2500, size: 80, speed: 0 },

{ spawnTime: 1200, displayTime: 2000, size: 70, speed: 0.5 },

{ spawnTime: 1000, displayTime: 1500, size: 60, speed: 1.0 },

{ spawnTime: 800, displayTime: 1200, size: 50, speed: 1.5 },

{ spawnTime: 600, displayTime: 1000, size: 40, speed: 2.0 },
...
return Math.min(Math.floor(score / 8) + 1, MAX_DIFFICULTY_LEVEL);
...
const newScore = Math.max(0, prev.score - 2);
...
if (newLevel !== stats.level && newLevel <= MAX_DIFFICULTY_LEVEL) {

const oldLevel = stats.level;

console.log(`LEVEL UP: ${oldLevel} -> ${newLevel}. Score: ${stats.score}`);

setStats(prev => ({ ...prev, level: newLevel }));

setCurrentDifficultySetting(DIFFICULTY_LEVELS_CONFIG[newLevel - 1]);

emitGameEvent('difficulty_change', { from: oldLevel, to: newLevel, newDifficultySettings: DIFFICULTY_LEVELS_CONFIG[newLevel - 1] });
